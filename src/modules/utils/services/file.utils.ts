export class FileUtils {
  ...
  static fileExtension(filename: string): string {
    ...
    return '.'.concat(splittedFile[splittedFile.length - 1]);
  }

  static getFileMimetype(fileExtension: string): string {
    return this.mimeTypes[fileExtension];
  }

  static getRelativeFolderName(folderName: string) {
    const splittedFile = folderName.split('/');
