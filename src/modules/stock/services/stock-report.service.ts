...
export class StockReportService {
  ...
  getResumedStockDataHeaders() {
    ...
    headers.push(['J2', 'KG']);
    headers.push(['K2', 'Dt. Prod.']);
    headers.push(['L2', 'Idade (Dias)']);
    headers.push(['M2', 'Dt. Venc.']);
    headers.push(['N2', 'Prazo (Dias)']);
    ...
    toExpiresData
      ...
      .forEach((item, index) =>
        values.push(
          ...
          [`K${row(index)}`, item.productionDate],
          [`L${row(index)}`, item.daysFromProduction],
          [`M${row(index)}`, item.dueDate],
          [`N${row(index)}`, item.daysToExpires],
        ),
      );
